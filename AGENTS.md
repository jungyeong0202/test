...
돈을 벌기가 너무 쉬웠다. 눌린 값에 사서 기준가에 파는 뻔한 전략이 **승률 95%,
시간당 82만원** 이었다 — 포켓몬 네 마리보다 많이 벌었다.

범인은 **평균 회귀**다. 기준가 아래면 무조건 위로 미는 힘이 붙어서 "싸다" 가
곧 "곧 오른다" 가 된다. **기준가를 흔들어 보는 것은 소용이 없었다**(82만 → 83만).
기준가가 어디에 있든 그 아래면 미는 힘이 붙기 때문이다. 힘 자체를 줄여야 한다
(`StockReversionScale`).

지금은 평균을 깎기보다 **진폭을 키우는** 쪽으로 맞춰 두었다. 거래 한 번의 결과:
...
- `StockExtremeShare` — 전체 시장 사건 중 극단 사건(±25~60%)의 몫.
- `StockScandalChance` — 값과 상관없이 그 자리에서 상장폐지되는 사건. 위기 경고를
  보고 팔고 나올 틈이 있으면 들고 있는 것이 안전해져서 도박이 아니게 된다.
- 이 셋을 건드리면 상장폐지 빈도도 함께 움직인다(회귀를 약하게 하면 값을 붙잡아
  주는 힘도 사라진다). 지금은 시간당 2.05회다.
