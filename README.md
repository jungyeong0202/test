...
피카츄는 41x49 도트에 걷기 4프레임, 나머지는 손으로 그린 작은 도트입니다.
...
기본으로 들어 있는 피카츄가 이 방식으로 만들어졌습니다 (41x49 도트, 14색, 4프레임).

```bash
python tools/import_sprite.py 피카츄.png --key pikachu --name 피카츄 --colors 14 \
    --part arm:0,18,5,27 --part lfoot:0,33,6,41 --part rfoot:12,42,21,48 \
    --motion "lfoot:0,0;2,-3;0,0;1,-1" \
    --motion "rfoot:0,0;1,-1;0,0;2,-3" \
    --motion "arm:0,0;0,1;0,0;0,-1" \
    --preview 확인용.png
```

- `--part 이름:x0,y0,x1,y1` 으로 움직일 덩어리를 지정합니다. 어느 사각형에도 안 들어간 픽셀은 `body` 로 묶입니다
- `--motion 이름:dx,dy;dx,dy;...` 는 프레임별 이동량입니다. 위 예시는 발을 번갈아 들고 팔을 흔드는 4프레임입니다
