...
우클릭 메뉴의 `주식시장 열기`를 누르면 별도 오버레이 창이 열립니다. 여섯 종목을 각각 한 주씩 매수하거나 매도할 수 있고, 최근 20회의 가격 변동을 선 그래프와 시작 가격 대비 등락률(%)로 볼 수 있습니다. 카드에는 안정형·성장형·고위험형 성격, 변동폭, 평균 매입가, 수수료를 반영한 내 수익률이 표시됩니다. 매수와 매도에는 각각 2% 수수료가 붙으며, 평가액도 실제 매도 예상액 기준입니다. 종목은 저마다 변동폭이 다르고 20초마다 종목별 호재·악재 뉴스 이벤트가 발생할 수 있습니다. 이벤트가 일어난 종목은 변동성 완화장치로 40초 동안 거래가 멈춥니다. 가격이 100원 아래로 떨어지면 해당 종목은 상장폐지되어 보유 주식이 소멸하고, 30분 뒤에는 성격이 무작위인 새 포켓몬 종목이 같은 자리에 상장됩니다. 오버레이 상단의 빨간 제목 영역을 끌어 원하는 위치로 옮길 수 있고, 보유금과 주식 평가액도 함께 확인할 수 있습니다. 돈과 보유 주식·가격은 저장됩니다. 이 기능은 게임용이며 실제 금융시장이나 현금과 연결되지 않습니다.
