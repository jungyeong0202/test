...
| `--check` | 창 없이 글자로만 자체 점검 (`PokemonTaskbar-debug.exe` 와 함께 씁니다) | — |
...
> `PokemonTaskbar-debug.exe --check` 를 직접 실행해도 같은 내용을 볼 수 있습니다.
> 이 콘솔 판은 창을 안 쓰기 때문에, GUI 판이 조용히 죽는 상황에서도 이유가 글자로 남습니다.
