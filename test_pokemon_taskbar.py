...
class EvolutionTest(unittest.TestCase):
    ...
    def test_stock_market_opens_in_its_own_overlay(self):
        ...
        self.assertNotIn(overlay.window.focus_get(), [row[0] for row in overlay.list_rows])
        overlay.select_stock(2, focus=True)
        self.app.root.update_idletasks()
        self.assertEqual(overlay.selected_index, 2)
        self.assertTrue(overlay.list_rows[2][0].cget("takefocus"))
        self.assertTrue(overlay.list_rows[2][0].bind("<Return>"))
